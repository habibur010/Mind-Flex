...
function Router() {
  ...
  const { data: onboardingStatus, isLoading: onboardingLoading } = useQuery<{ completed: boolean }>({
    queryKey: ["/api/onboarding/status"],
    enabled: !!user,
  });

  useEffect(() => {
    if (user && onboardingStatus && !onboardingStatus.completed && location !== "/onboarding") {
